...
export default function ImmersiveMuseum() {
  ...
  useEffect(() => {
    ...
    const titleEl = root.querySelector('#artifact-title')
    const descriptionEl = root.querySelector('#artifact-description')
    const timelineScale = root.querySelector('#timeline-scale')
    const canvas = root.querySelector('#three-canvas')
    if (!epochEl || !titleEl || !descriptionEl || !timelineScale || !canvas) return
    ...
    function buildTimeline() {
      ...
      timelineItems.forEach((item, index) => {
        ...
        point.addEventListener('click', () => {
          ...
          if (!trigger) return
          const y = trigger.start + ratio * (trigger.end - trigger.start)
    ...
    function setHeroVisibility(visible) {
      if (!state.heroAnimating && visible === state.heroVisible) return
      if (state.heroAnimating && visible === state.heroTargetVisible) return
    ...
    async function buildCards() {
      const textures = await Promise.all(timelineItems.map((item) => createCardTexture(item.image, item.title)))
      textures.forEach((tex) => ownedTextures.push(tex))
    ...
    function render() {
      ...
      if (timelineCursor) timelineCursor.style.left = `${state.progress * 100}%`
      if (timelineProgress) timelineProgress.style.width = `${state.progress * 100}%`
      ...
      cardsGroup.children.forEach((plane) => {
        ...
        if (near > 0.72) opacity = 1
        if (worldZ > 0.6) {
          const pastCameraFade = Math.max(0, 1 - (worldZ - 0.6) / layout.postCameraFade)
      ...
      if (heroShouldShow) {
        if (heroCandidateIndex !== state.activeIndex) {
          if (state.heroVisible || state.heroAnimating) {
            ...
            if (state.heroVisible) setHeroVisibility(false)
          } else {
  ...
  return (
    <div className="experience-route" ref={rootRef}>
      <main className="experience" id="experience">
        <canvas id="three-canvas" aria-label="3D immersive museum timeline" />
        ...
        <section className="hero-copy" aria-live="polite">
          <p className="epoch" id="epoch">986-1001</p>
          <h1 id="artifact-title">The Pillow Book</h1>
          <p id="artifact-description" className="artifact-description">
            A long-form diary tradition that established serialized personal publishing centuries before modern blogs.
          </p>
        </section>

        <section className="timeline-wrap" aria-label="Museum timeline">
          <div className="timeline-scale" id="timeline-scale" />
          <div className="timeline-footer">
            <p className="chapter">Rise of Blogging</p>
            <p className="hint">Scroll to travel deeper into the gallery</p>
          </div>
        </section>
      </main>
    </div>
