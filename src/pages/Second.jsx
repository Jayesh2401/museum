...
const TIMELINE_ITEMS = [
  { title: 'Rise Of Blogging', year: '986 - 2000', image: 'https://images.unsplash.com/photo-1504198266285-165a74cfde2c?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Wordpressism', year: '1998 - 2004', image: 'https://images.unsplash.com/photo-1579783901586-d88db74b4fe4?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Myspacism', year: '2003 - 2006', image: 'https://images.unsplash.com/photo-1523726491678-bf852e717f6a?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Facebook Art', year: '2004 - present', image: 'https://images.unsplash.com/photo-1460661419201-fd4cecdf8a8b?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Youtubism', year: '2005 - present', image: 'https://images.unsplash.com/photo-1545239351-1141bd82e8a6?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Twitt Art', year: '2006 - present', image: 'https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Instagraminism', year: '2010 - present', image: 'https://images.unsplash.com/photo-1461749280684-dccba630e2f6?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Twitch Art', year: '2011 - present', image: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=1400&q=80' },
  { title: 'Tiktokko', year: '2016 - present', image: 'https://images.unsplash.com/photo-1517048676732-d65bc937f952?auto=format&fit=crop&w=1400&q=80' }
]

const DEFAULT_CONFIG = {
  frameCount: TIMELINE_ITEMS.length,
  frameWidth: 4.94,
  frameHeight: 8,
  frameDepth: 0.2,
  frameInset: 0.2,
...
function createPanelTextTexture(title, year) {
  const canvas = document.createElement('canvas')
  canvas.width = 1024
  canvas.height = 360
  const ctx = canvas.getContext('2d')
  ...
  ctx.font = '700 72px "Trebuchet MS", "Avenir Next", sans-serif'
  ctx.fillText(title, canvas.width / 2, 126)

  ctx.fillStyle = '#2f3645'
  ctx.font = '500 52px "Trebuchet MS", "Avenir Next", sans-serif'
  ctx.fillText(year, canvas.width / 2, 212)
...
export default function Second() {
  ...
  useEffect(() => {
    ...
    function rebuildScene(cfg) {
      ...
      for (let i = 0; i < cfg.frameCount; i += 1) {
        ...
        const item = TIMELINE_ITEMS[i % TIMELINE_ITEMS.length]
        ...
        const imageTexture = getImageTexture(item.image)
        ...
        const panelTextTexture = createPanelTextTexture(item.title, item.year)
        if (panelTextTexture) panelTextTextures.push(panelTextTexture)
        const panelText = new THREE.Sprite(
          new THREE.SpriteMaterial({
            map: panelTextTexture,
            transparent: true,
            depthWrite: false
          })
        )
        panelText.position.set(0, domeHeight * 0.64, 0.06)
        panelText.scale.set(domeWidth * 0.72, domeHeight * 0.24, 1)

        holder.add(face)
        holder.add(waterMesh)
        holder.add(panelText)
        frameGroup.add(holder)
        panelHolders.push({ holder, itemIndex: i })
      }
    ...
    const tick = (now) => {
      ...
      let bestIndex = 0
      let bestX = Infinity
      for (const panel of panelHolders) {
        panel.holder.getWorldPosition(tmp)
        tmp.project(camera)
        if (tmp.z < 1 && tmp.z > -1) {
          const dx = Math.abs(tmp.x)
          if (dx < bestX) {
            bestX = dx
            bestIndex = panel.itemIndex
          }
        }
      }
    ...
    sceneDataRef.current = {
      ...
      settings: {
        ...
        rotateToIndex(index) {
          spin.targetRotation = index * angleStepRef
        }
      },
      dispose() {
        ...
        panelTextTextures.forEach((texture) => texture.dispose?.())
  ...
  return (
    <div className="app-shell second-shell">
      <div className="second-topbar">
        <div className="second-brand">
          <span>Museum Of</span>
          <strong>Digital Influence</strong>
        </div>
        <div className="second-actions">
          <button type="button">About</button>
          <button type="button">Support Project</button>
          <a href="#/" className="route-link-btn">Routes</a>
        </div>
      </div>

      <div className="second-active-copy">
        {/* <h2>{activeItem.title}</h2> */}
        {/* <p>{activeItem.year}</p> */}
      </div>

      <div className="second-timeline-wrap">
        <div className="second-timeline-line" />
        <div className="second-timeline-grid">
          {TIMELINE_ITEMS.map((item, idx) => (
            <button
              key={`${item.title}-${item.year}`}
              type="button"
              className={`second-timeline-item ${activeIndex === idx ? 'active' : ''}`}
              onClick={() => sceneDataRef.current?.settings?.rotateToIndex(idx)}
            >
              <span className="dot" />
        ...
        <div className="second-scroll-hint">Scroll To Navigate</div>
      </div>
